...
// ✅ UPDATED SUPPORT FUNCTION (already integrated)
function showSupport(){
  const messages = [
    "Take a slow breath 🌬️",
...
function updatePrompt(){
  const box = document.getElementById("dailyPrompt");
  box.textContent = prompts[Math.floor(Math.random() * prompts.length)];
}
...
function exportCSV(){
  ...
  entries.forEach(e => {
    csv += `${e.date},${e.mood},${e.note.replace(/,/g," ")}\n`;
  });
...
if("Notification" in window){
  ...
  setTimeout(() => {
    new Notification("CalmSpace Reminder", {
      body: "Time to log your mood today 🌿"
    });
  }, 5000);
}
